...
@section('content')
  ...
  <section class="relative py-10 stripe-bg">
    ...
    @include('components.breadcrumb', ['items' => [
      ...
      ['label' => 'Organisasi', 'url' => route('organization.index')],
      ['label' => $item->name]
    ]])
    <div class="grid md:grid-cols-3 gap-6">
      <div>
        <div class="aspect-[4/5] rounded-3xl overflow-hidden border border-white/40 bg-gray-100 shadow-lg">
          <img loading="lazy" class="w-full h-full object-cover" src="{{ $item->image_path ? Storage::url($item->image_path) : 'https://picsum.photos/seed/org'.$item->id.'/600/750' }}" alt="{{ $item->name }}">
        </div>
