...
@section('content')
  ...
  <section class="relative py-10 overflow-hidden">
    ...
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-semibold text-primary">Kelola UKK</h1>
      <div class="flex items-center gap-3">
        ...
        <a href="{{ route('admin.ukk.create') }}" class="bg-primary text-white px-4 py-2 rounded">+ Buat UKK</a>
      </div>
    </div>
          ...
          @forelse($items as $it)
            <tr class="border-t">
              ...
              <td class="p-3 text-right space-x-3">
                <a href="{{ route('admin.ukk.edit', $it) }}" class="text-primary hover:underline">Edit</a>
                <form action="{{ route('admin.ukk.destroy', $it) }}" method="post" class="inline" onsubmit="return confirm('Hapus UKK ini?')">
                  @csrf
                  @method('DELETE')
